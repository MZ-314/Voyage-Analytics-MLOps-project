...
# API endpoints
API_URL = "http://localhost:8080/predict"  # Kind with port-forward
...
with st.sidebar:
    ...
    try:
        ...
        if response.status_code == 200:
            st.success("✅ API: Online (Kind)")
            api_status = True
            api_url = API_URL
        else:
            raise Exception("Kind API not responding")
    except:
        try:
...
# Page: About
    ...
    **Voyage Analytics** is an end-to-end MLOps project demonstrating the complete lifecycle of 
    ...
    - **API:** Flask, Docker, Kubernetes (Kind)
